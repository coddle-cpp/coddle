...
{
  setenv("PKG_CONFIG_PATH", makePath(".coddle", "libs", "usr", "local", "lib", "pkgconfig").c_str(), 1);
  static std::unordered_map<std::string, std::function<void(Config &config, const std::string &version)>> libs = {
    {
      ...
      {
        if (!isFileExist(makePath(".coddle", "libs", "usr", "local", "lib", "pkgconfig", "jsoncpp.pc")))
        {
    ...
    },
    {
      ...
      {
        if (!isFileExist(makePath(".coddle", "libs", "usr", "local", "lib", "pkgconfig", "libcurl.pc")))
        {
    ...
    },
    {
      ...
      {
        if (!isFileExist(makePath(".coddle", "libs", "usr", "local", "lib", "pkgconfig", "sdl2.pc")))
        {
